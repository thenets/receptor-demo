...
def receptor_mesh(base_tmp_dir):
    class ReceptorMeshSetup:
        ...
        def setup(self, mesh_name: str = "simple", socket_file_name: str = "node1.sock"):
...
def receptor_bin_path():
    ...
    # Check if the receptor binary is in '../bin' and returns
    ...
    try:
        ...
        raise Exception(
            "Receptor binary not found in $PATH or in '../bin'"
        )
...
@pytest.fixture(scope="class")
def receptor_mesh_simple(
    receptor_bin_path,
    receptor_nodes,
    receptor_mesh,
):
    # Set custom config files dir
    receptor_mesh.setup("simple")
